...
const businessDataKeys = new Set([
  ...
  "paymentReconciliation", "priceSync", "removedInventory",
]);
const jarvisDataKeys = new Set([
  "jarvisSessions", "jarvisEmails", "jarvisEmailFeedback", "jarvisEmailActions", "jarvisCalendarEvents",
  "jarvisTasks", "jarvisGoogleTokens", "jarvisCalendarTokens", "jarvisOAuthStates", "jarvisDiagnostics",
  "jarvisActivity",
]);
...
async function readDb() {
  ...
  try {
    ...
    const migration = jarvisDataMigration(db);
    if (migration.unknownKeys.length) {
      log("warn", "database.unknown_data_preserved", { keys: migration.unknownKeys });
    }
    if (migration.removedKeys.length) {
      await writeDbBackup(db, { force: true, reason: "pre-legacy-assistant-cleanup" });
      db = migration.db;
    }
    ...
    if (migration.removedKeys.length) {
      await writeDb(db);
      log("info", "database.legacy_data_removed", { removedKeyCount: migration.removedKeys.length });
    }
...
function jarvisDataMigration(db) {
  const keys = Object.keys(db);
  const removedKeys = keys.filter((key) => jarvisDataKeys.has(key));
  const unknownKeys = keys.filter((key) => !businessDataKeys.has(key) && !jarvisDataKeys.has(key));
  return {
    db: Object.fromEntries(Object.entries(db).filter(([key]) => !jarvisDataKeys.has(key))),
    removedKeys,
    unknownKeys,
  };
}
