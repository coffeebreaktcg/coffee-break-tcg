...
| Google OAuth | `GOOGLE_OAUTH_CLIENT_SECRET` | Remplacer dans Google Cloud et Render. Les connexions Gmail/Calendar peuvent devoir être réautorisées. |
...
| Jarvis | `JARVIS_PASSWORD_HASH` | Remplacer le hash. Invalider `jarvisSessions` en cas de compromission. |
| Chiffrement Jarvis | `JARVIS_TOKEN_SECRET` | Choisir une valeur aléatoire dédiée. Changer cette valeur rend les jetons Google stockés illisibles ; reconnecter Gmail et Calendar. |
